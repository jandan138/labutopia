...
`S2F2_VELOCITY_CONTACT_OFFSET=COMPLETE_GO_NEXT`，`S2F5_PROMOTION_REVIEW=NEXT`。
这表示 S2F2 找到了一个 promotion review 候选，但仍不允许直接跑 S3。
...
产品口径：S2F2 已经找到一个静态持液 promotion review 候选
`C2A_009_S2F2_VEL020`，但它不是直接 S3 倒液候选。当前下一步是
`S2F5_PROMOTION_REVIEW`，要做跨 seed 和跨粒子数量复核；`s3_kinematic_pour_released=false`。
`s2f2_initial_layout_hash_audit` 还显示三个 parent 都有 post-reset hash variation：USD authored
spawn position 是固定的，但 PhysX reset / settle 后 contact 参数会带来轻微初始状态差异。因此
contact-offset 组只作为诊断证据，不直接升级为 release evidence。正式 manifest 的 `next_stage`
也写明 `promotion_caveat=COUPLED_DIAGNOSTIC_REQUIRES_INITIAL_LAYOUT_RETEST`，所以 S2F5 第一项必须复测
initial-layout hash 稳定性。
