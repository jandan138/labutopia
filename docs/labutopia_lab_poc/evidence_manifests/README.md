...
s2f1_best_for_s3=[]
s2f2_velocity_contact_offset_next=true
...
`S2F2_VELOCITY_CONTACT_OFFSET=NEXT`，所以仍不允许直接跑 S3。
...
产品口径：`C2A_005` 和 `C2A_009` 是 near-pass candidates，不是 S3 candidates。
`best_for_s3=[]` 且 `s3_kinematic_pour_released=false`；当前下一步是
`S2F2_VELOCITY_CONTACT_OFFSET`，不是 `S3_KINEMATIC_POUR`。
