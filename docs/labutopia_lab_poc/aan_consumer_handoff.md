...
- 新的 ConvertAsset AAN package 已经通过 package-local material closure。

后续 PM 汇报必须带上证据线名称，避免把旧 overlay 的限制误套到 AAN package，或把
AAN package 的闭环反推到旧 overlay。ConvertAsset AAN 现在把资产包线补上了：

- package 内有 `Aluminum_Anodized_Charcoal.mdl` 等 MDL mirror；
- package 内有 texture mirror；
- manifest 中 `material_closure=pass`；
...
这次 AAN package 的 `material_closure=pass` 可以汇报，但不能升级成 full visual parity。

三层话术如下：

| Term | 可以说 | 不能说 |
|---|---|---|
| `material_closure` | AAN package material gate passed；package-local material evidence 已记录。 | full visual parity 已证明。 |
...
> AAN package material gate 已通过，remote material dependencies 已按 manifest 做 local
> mirror；这证明 package 可离线消费并保留材质来源证据，不代表 source-native full
> material closure 或 full visual parity。
...
4. 单独规划 visual/material parity follow-up：`Beaker_01` 最终 smoke 仍有
   `mdl_compiler_error_count=8` 的 OmniGlass MDL compiler warning；这不阻塞 Stage 4b
   smoke PASS，但阻止 full visual material parity / source-native material closure 声明。
