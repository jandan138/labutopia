...
| S2F Collider Follow-up | 怎么把 S2 失败收敛成一个可持液 collider？ | 已完成到 S2F4：C2 proxy、C3 SDF、C4 native isolation 都没有放行 S3 | 参数/几何/cooking/native mesh 失败归因 |
...
原始不完整的 `/World/fluid`、`/World/ParticleSet`、`/World/ParticleSystem` 关掉，另起一套完整 PBD overlay：
