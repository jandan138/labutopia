...
def test_build_d4_wrapper_promotion_matrix_is_12_trials_with_pinned_init():
    ...
    assert all(c.wrapper_collider_mode == "box_cup" for c in candidates)
    ...
    assert spec0.wrapper_collider_mode == "box_cup"
    assert spec0.collision_approximation == "box"
    assert spec0.collider_count == 5
    ...
    # Seal1-winning band restored (seal3 cco/crest/arc regression reverted).
    assert cfg_1k.particle_width == pytest.approx(0.0035)
    assert cfg_1k.collider_contact_offset == pytest.approx(0.004)
    assert cfg_1k.collider_rest_offset in (None, 0.0)
